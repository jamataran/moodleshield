 ...
 * hacer puede destruir datos, y lo corta antes de que ocurra. Va enganchado como
 * hook `PreToolUse` en `.claude/settings.json`.
 *
 * Es configuración del agente, no código del producto: no entra en `src/`, no
 * cambia el comportamiento de la aplicación y quitarlo no rompe nada. La
 * decisión es una función pura (`revisar`) para poder comprobarla sin ejecutar
 * nada:
 *
 *   echo '{"tool_name":"Bash","tool_input":{"command":"…"}}' | node .claude/hooks/guardia-datos.mjs
 *
 * Sale 0 si deja pasar y 2 —con la explicación por stderr— si corta.
 *
